...
var convertRegexToString = function(regex) {
    ...
    temp = S(temp).replaceAll('/i', '').replaceAll('.', '').replaceAll('/','').replaceAll('*', '').s;
    return temp;
};
/*
...
exports.lint = function(value, nameStartsWith, type) {
    ...
    if (value.search(nameStartsWith) == -1) {
        grunt.log.error(value + " does not start with " + nameStartsWith);    
       ...
       grunt.log.ok(value + " starts with " + nameStartsWith);
